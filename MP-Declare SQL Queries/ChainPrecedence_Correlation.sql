...
SELECT 'chain precedence' AS [Constraint], TaskA, TaskB, a.Param1, b.Param2,
	...
	WHERE a.Task = x.TaskB AND b.Task = x.TaskB AND a.Param1 = b.Param1 AND b.ID IN(SELECT TOP 1 ID
														FROM [Log] b
														WHERE b.Task = x.TaskA AND b.Instance = a.Instance AND b.[Time] < a.[Time])
														
										  AND NOT EXISTS(SELECT *
													     FROM [Log] b, [Log] c
													     WHERE c.Instance = a.Instance AND b.Instance = a.Instance AND b.Task = x.TaskA AND c.[Time] < a.[Time] AND c.[Time] > b.[Time])
